...
extension SetUpLocationDrivenGeotriggersView {
    ...
    class Model: ObservableObject {
        ...
        @Published private(set) var hasCurrentSection = false
        
        /// A Boolean value indicating whether there is a point-of-interest.
        @Published private(set) var hasPointOfInterest = false
        
        /// A dictionary for nearby features.
        @Published private(set) var nearbyFeatures: [String: ArcGISFeature] = [:]
        
        /// The popup to show the feature information.
        @Published private(set) var popup: Popup?
        
        /// The fence geotrigger notification info status.
        @Published private(set) var fenceGeotriggerStatus: FenceGeotriggerStatus = .notSet
        ...
        private let locationDataSource = SimulatedLocationDataSource(polyline: .walkingTour())
        ...
        private var currentSectionName: String? {
            featureNamesInFenceGeotrigger[.sectionFenceGeotrigger]?.last
        }
        
        /// The names of nearby point-of-interest features.
        private var nearbyPOINames: [String] {
            featureNamesInFenceGeotrigger[.poiFenceGeotrigger] ?? []
        }
        ...
        func makeGeotriggerMonitors() -> [GeotriggerMonitor] {
            ...
            guard let operationalLayers = map.operationalLayers as? [FeatureLayer],
                  ...
                  let gardenSectionsTable = gardenSectionsLayer.featureTable as? ServiceFeatureTable,
                  let gardenPOIsTable = gardenPOIsLayer.featureTable as? ServiceFeatureTable
            else {
                return []
            }
            ...
            let gardenSectionsGeotriggerMonitor = makeFenceGeotriggerMonitor(
                feed: geotriggerFeed,
                featureTable: gardenSectionsTable,
                bufferDistance: 0.0,
                fenceGeotriggerName: .sectionFenceGeotrigger
            )
            let gardenPOIsGeotriggerMonitor = makeFenceGeotriggerMonitor(
                feed: geotriggerFeed,
                featureTable: gardenPOIsTable,
                bufferDistance: 10.0,
                fenceGeotriggerName: .poiFenceGeotrigger
            )
            
            return [gardenSectionsGeotriggerMonitor, gardenPOIsGeotriggerMonitor]
        }
        
        /// Creates a fence geotrigger monitor.
        /// - Parameters:
        ...
        private func makeFenceGeotriggerMonitor(
            feed: GeotriggerFeed,
            featureTable: ServiceFeatureTable,
            bufferDistance: Double,
            fenceGeotriggerName: String
        ) -> GeotriggerMonitor {
        ...
        func handleGeotriggerNotification(_ fenceNotificationInfo: FenceGeotriggerNotificationInfo) {
            ...
            let notificationType = fenceNotificationInfo.fenceNotificationType
            ...
            case .entered:
                ...
                // Update the status to notify the view to update.
                fenceGeotriggerStatus = .entered(featureName: featureName)
            case .exited:
                // The user left the geofence: remove the feature from the dicts.
                // Enter and exit events follow a first-in-last-out order, so the
                ...
                fenceGeotriggerStatus = .exited(featureName: featureName)
            @unknown default:
                fatalError("Unexpected fence notification type.")
            }
        }
        
        /// Sets the popup to show the information of the current garden section.
        func setSectionPopup() {
            guard let currentSectionName,
                  let sectionFeature = nearbyFeatures[currentSectionName] else { return }
            popup = Popup(geoElement: sectionFeature)
        }
        
        /// Sets the popup to show the information of the first nearby POI.
        func setPOIPopup() {
            guard let name = nearbyPOINames.first,
                  let poiFeature = nearbyFeatures[name] else { return }
            popup = Popup(geoElement: poiFeature)
        }
        
        /// Creates a location display from a location data source.
        /// - Parameter dataSource: The `LocationDataSource` to create the location display from.
        /// - Returns: A new `LocationDisplay` object.
        private static func makeLocationDisplay(dataSource: LocationDataSource) -> LocationDisplay {
...
private extension String {
    ...
    static let sectionFenceGeotrigger = "Section Fence Geotrigger"
    
    /// The point-of-interest geotrigger name.
    static let poiFenceGeotrigger = "POI Fence Geotrigger"
}
