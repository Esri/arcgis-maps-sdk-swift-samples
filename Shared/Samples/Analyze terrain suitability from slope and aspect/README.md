...
6. Convert the final result to a `DiscreteFieldFunction`.
7. Create a `FieldAnalysis` from the discrete field function.
8. Apply a `ColormapRenderer` with a color for areas not matching the terrain suitability criteria, and a color for matching areas.
9. Add the analysis to an `AnalysisOverlay`.
