...
extension AnalyzeTerrainSuitabilityFromSlopeAndAspectView {
    ...
    @Observable
    final class Model {
        /// The WGS 84 UTM zone 30N spatial reference used by the elevation data
        /// and map.
        ...
        /// The analysis associated with the selected scenario, if it has been
        /// created.
        ...
        /// Creates the elevation field and terrain suitability analyses used by
        /// the sample.
        /// - Throws: An error if the elevation data cannot be accessed or used
        ///   to create a field.
        func configureTerrainSuitabilityAnalyses() async throws {
            isUpdatingAnalysis = true
            defer { isUpdatingAnalysis = false }
            
            // Project the Web Mercator raster into UTM30N because slope and
            // aspect require a conformal projection.
            ...
            let elevationFunction = ContinuousFieldFunction.function(
                withResult: elevationField
            )
            let inputs = TerrainAnalysisInputs(
                ...
                aboveSeaLevelSelection: elevationFunction
            ...
            updateAnalysisVisibility()
            viewpoint = Viewpoint(
                center: elevationField.extent.center,
                scale: 200_000
            )
        }
        ...
        private func updateAnalysisVisibility() {
            // Reset; MapView's analysis view state callback will set this to
        ...
        /// terrain criteria.
        /// - Parameters:
        ///   - inputs: The elevation-derived field functions used to evaluate
        ///     each cell.
        ///   - criteria: The slope, aspect, and elevation ranges that define
        ///     suitable terrain.
        ///   - color: The color used to render cells that satisfy all criteria.
        /// - Returns: A hidden field analysis configured with the suitability
        ...
        private func makeAnalysis(
            ...
            // Create Boolean masks that select cells within the inclusive
            ...
            // Intersect the masks so a cell is suitable only when it meets
            // every criterion. Remove cells below sea level, then convert the
            // Boolean result to discrete values for rendering.
            ...
            // Render false cells in white and suitable cells in the scenario's
            // color. The analysis starts hidden because
            // updateAnalysisVisibility() controls which scenario is displayed.
        ...
        /// - Parameters:
        ///   - aspectFunction: A function that calculates aspect values in
        ///     degrees clockwise from north.
        ///   - start: The clockwise starting angle of the aspect range.
        ///   - end: The clockwise ending angle of the aspect range.
        /// - Returns: A Boolean function that selects cells within the aspect
        ///   range.
        private func makeAspectRangeMask(
            ...
            if start <= end {
                ...
                // A range whose start is greater than its end crosses north,
                // so join the selections on either side of 0 degrees.
    ...
    enum SiteScenario: CaseIterable, Identifiable {
        ...
        var description: String {
            ...
            case .gentleSouthFacingSlopes:
                """
                Finds sheltered, lowland terrain with gentle south-facing \
                slopes.
                """
            case .steepWestAndNorthFacingSlopes:
                """
                Finds exposed, upland terrain with steep west- through \
                north-facing slopes.
                """
            }
        }
...
private struct TerrainCriteria {
    ...
    /// The clockwise starting angle of the suitable aspect range, in degrees
    /// from north.
    let aspectStart: Float
    /// The clockwise ending angle of the suitable aspect range, in degrees
    /// from north.
    let aspectEnd: Float
...
private extension URL {
    /// A URL to the local GeoTIFF elevation raster of the Isle of Arran,
    /// Scotland.
